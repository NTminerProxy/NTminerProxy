...
SERVICE_NAME="NTminerProxyservice"

PATH_RUST="/root/NTminerProxy"
PATH_EXEC="NTminerProxy"
...
disable_autostart() {
    ...
    if [ "$(command -v systemctl)" ]; then
        ...
        sudo sed -i '/\/root\/NTminerProxy\/NTminerProxy\ &/d' /etc/rc.local
    fi
